...
func (s *interactshScanner) Scan(ctx context.Context, target string, options map[string]string) ([]types.Finding, error) {
	...
	s.client.StartPolling(5*time.Second, func(interaction *server.Interaction) {
		s.logger.Debug("Received interaction", "interaction", interaction)
	})
	...
	// Note: GetInteractions is not available in the current client version
	// The interactions are handled via the callback in StartPolling
	for uniqueID, payload := range s.payloads {
		...
		if false { // Placeholder condition
			finding := types.Finding{
				...
				Evidence: fmt.Sprintf("UniqueID: %s\nTarget: %s\nTest Case: %s",
...
func (s *interactshScanner) testSSRF(ctx context.Context, target string, options map[string]string) []types.Finding {
	...
	_, _ = url.Parse(target) // Parse target URL for potential future use
